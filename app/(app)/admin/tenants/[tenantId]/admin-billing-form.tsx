...
export function AdminBillingForm({ billingInfo, tenantId, ownerProfile }: { billingInfo: any, tenantId: string, ownerProfile: any }) {
    ...
    async function handleSubmit(formData: FormData) {
        setLoading(true);
        try {
            const result = await updateTenantBillingInfo(formData);
            if (result?.error) {
                toast({
                    variant: "destructive",
                    title: "Hata",
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-white rounded-xl border border-slate-200 shadow-sm">
                ...
                <div className="space-y-4 col-span-1">
                    <div className="space-y-2">
                        <Label>İşletme Sahibi (Read-Only)</Label>
                        <Input disabled value={ownerProfile?.full_name || "Bilinmiyor"} className="bg-slate-50" />
                    </div>
                        ...
                        {billingType === "company" ? (
                            ...
                            <>
                                <div className="space-y-2">
                                    ...
                                    <Input name="fullName" defaultValue={billingInfo?.full_name || ownerProfile?.full_name} placeholder="Ad Soyad" />
                                </div>
                                <div className="space-y-2">
                                    <Label>TCKN</Label>
