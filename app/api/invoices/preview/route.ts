// Mail preview route - fatura email şablonunu tarayıcıda görüntüleme
import { NextRequest, NextResponse } from "next/server";

export async function GET(request: NextRequest) {
    const searchParams = request.nextUrl.searchParams;
    const recipientName = searchParams.get("name") || "Test İşletme";
    const planName = searchParams.get("plan") || "UppyPro Inbox";
    const amount = searchParams.get("amount") || "₺1.074,00";
    const paymentDate = searchParams.get("date") || "20 Şubat 2026";

    const logoUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/brand-logo-text.png`;
