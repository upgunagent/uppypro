 ...
 * Uses pdfmake's instance API with VFS fonts - zero filesystem access.
 * Locally tested and confirmed working: 14KB PDF with Turkish characters.
 */
export async function generatePdfBuffer(data: AgreementPdfData): Promise<Buffer | null> {
    try {
        // pdfmake exports an INSTANCE (not a constructor)
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const pdfmake = require('pdfmake');
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const vfsModule = require('pdfmake/build/vfs_fonts');

        // Write font data (base64) into pdfmake's virtual filesystem as Buffers
        const fontNames = ['Roboto-Regular.ttf', 'Roboto-Medium.ttf', 'Roboto-Italic.ttf', 'Roboto-MediumItalic.ttf'];
        for (const fontName of fontNames) {
            const fontData = vfsModule[fontName];
            if (fontData) {
                pdfmake.virtualfs.writeFileSync(fontName, Buffer.from(fontData, 'base64'));
            }
        }

        // Register font descriptors (names resolve through VFS, not filesystem)
        pdfmake.setFonts({
            Roboto: {
        ...
        console.log('[PDF Generator] Fonts loaded via VFS (zero filesystem access)');
        ...
        const docDefinition: TDocumentDefinitions = {
            ...
            content: [
                ...
                {
                    table: { widths: ['*'], body: [[{ stack: buyerLines, margin: [6, 6, 6, 6] }]] },
                ...
                { text: ['Sözleşme\'nin konusu; Alıcı\'nın seçtiği ', { text: data.plan.name, bold: true }, ' paketi kapsamında dijital hizmet aboneliğinin ifası ve bunun karşılığında abonelik bedelinin tahsili ile tarafların hak ve yükümlülüklerinin düzenlenmesidir.'], margin: [0, 0, 0, 10] },
        ...
        // Use pdfmake instance API: createPdf() then getBuffer()
        const pdfResult = pdfmake.createPdf(docDefinition);
        const buffer = await pdfResult.getBuffer();
        console.log(`[PDF Generator] PDF created successfully: ${buffer.length} bytes`);
